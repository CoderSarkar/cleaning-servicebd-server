...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.ydy8u.mongodb.net/${process.env.DB_NAME}?retryWrites=true&w=majority`;
...
const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true });
client.connect(err => {
    const serviceCollection = client.db("mrCleaner").collection("service");
    const bookingOrderCollection = client.db("mrCleaner").collection("orderService");
    const adminCollection = client.db("mrCleaner").collection("adminList");
    const reviewCollection = client.db("mrCleaner").collection("reviewList");
